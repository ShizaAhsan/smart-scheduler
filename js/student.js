// Student portal logic for Campus Smart Scheduler
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Student default cohort is "3rd Semester" (for Ali Raza)
    const semester = "3rd Semester";

    if (path.includes('dashboard.html')) {
        initDashboard(currentUser, semester);
    } else if (path.includes('courses.html')) {
        initCourses(semester);
    } else if (path.includes('timetable.html')) {
        initTimetable(semester);
    } else if (path.includes('announcements.html')) {
        initAnnouncements();
    } else if (path.includes('profile.html')) {
        initProfile(currentUser, semester);
    }
});
...
// 1. Dashboard
function initDashboard(student, semester) {
    ...
    .then(r => r.json())
    .then(courses => {
        const myCourses = courses.filter(c => c.semester === semester);
        if (statNums[0]) statNums[0].textContent = myCourses.length;
    });

    // Keep Assignments & Exams static or default
    if (statNums[1]) statNums[1].textContent = 2; // Assignments
    if (statNums[2]) statNums[2].textContent = 2; // Exams
    ...
    if (classesContainer) {
        ...
        fetch(`../api/timetable?semester=${semester}`)
        .then(r => r.json())
        .then(classes => {
            ...
            if (todayClasses.length === 0) {
                ...
                noClass.textContent = `☕ No classes scheduled for today (${todayName}). Enjoy your day!`;
            ...
            todayClasses.forEach((cls, idx) => {
                ...
                item.innerHTML = `
                    ...
                    <div class="class-info">
                        ...
                        <div class="class-sub">Room ${cls.room_num} · Professor: ${cls.teacher_name}</div>
                    </div>
...
function initCourses(semester) {
    ...
    .then(courses => {
        const myCourses = courses.filter(c => c.semester === semester);
        card.innerHTML = `<h3 class="card-title">📚 My Courses (${semester})</h3>`;
        
        if (myCourses.length === 0) {
            card.innerHTML += `<p style="padding:1rem; color:#888;">No courses scheduled for your semester currently.</p>`;
            return;
        }
        ...
        myCourses.forEach(c => {
            ...
            cCard.innerHTML = `
                ...
                <div style="font-size: 13px; color:#64748b; margin-top:8px;">
                    Department: <strong>${c.department}</strong><br/>
                    Type: <strong>${c.type}</strong><br/>
                    Credits: <strong>${c.credits} Credit Hours</strong><br/>
                    Professor: <strong>${c.teacher_name || 'TBA'}</strong>
                </div>
            `;
...
function initTimetable(semester) {
                ...
                if (classItem) {
                    ...
                    td.innerHTML = `
                        <div class="tt-class ${color}">
                            ...
                            <div class="tt-class-info">${classItem.room_num} (${classItem.teacher_name})</div>
                        </div>
                    `;
...
function initAnnouncements() {
    ...
    .then(announcements => {
        ...
        announcements.forEach(a => {
            ...
            const badgeText = a.role_target === 'all' ? 'Notice' : 'Student';
...
function initProfile(student, semester) {
    const profileWrap = document.querySelector('.profile-wrap');
    if (!profileWrap) return;

    // Load initial values
    updateProfileDOM(student, semester);
    ...
    if (editBtn) {
        editBtn.addEventListener('click', (e) => {
            e.preventDefault();
            openEditProfileModal(student);
        });
    }
}

function updateProfileDOM(user, semester = '3rd Semester') {
    ...
    if (infoValues.length >= 3) {
        ...
        infoValues[2].textContent = `CS-2026-${user.id}`; // Roll No
    }
}
